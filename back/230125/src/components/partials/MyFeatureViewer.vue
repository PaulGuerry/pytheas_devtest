<template lang="html">
    <p class="pt-[5px] mt-[100px] px-2 text-2xl text-left text-blue-700 font-light"> {{ this.proteinName }}</p>
    <div id="myfv"> </div>
</template>
        ...
        this.proteinData[0].features.forEach(element => {
            if (element.type.includes("MOTIF") || element.type.includes("REGION")) {
                ...
                if (element.alternativeSequence.length > 0) {
                    variantData.push({x: parseInt(element.begin), y: parseInt(element.end), description: "> " + element.alternativeSequence + " " + element.description});
                } else {
                    variantData.push({x: parseInt(element.begin), y: parseInt(element.end), description: "> MISSING " + element.description});
                }
            }
        });
        var featureA = [ 
