<template lang="html">
    <div  class="w-full py-4 text-xl text-gray-600 col-start-1 col-span-5"> 
        ...
        <div class=" w-1/4"></div>
        <div class="chart-container w-1/2"><VuePlotly :data="boxAgeAtFirstSymp" :layout="boxLayout_alt" :config="plotConfig"></VuePlotly></div>
        <div class=" w-1/4"></div>
        </div>
    </div>
    ...
    methods: {
        ...
        initialState() {
            return {
                ...
                boxData_alt: this.boxAgeAtFirstSymp,
        ...
        gatherStats() {
            ...
            this.stats.all.median = Math.round((this.stats.all.median * 100) * (1 + Number.EPSILON)) / 100
            this.stats.all.iqr = stats_disease[0].age_at_first_symp.all.iqr
            this.stats.all.iqr = Math.round((this.stats.all.iqr * 100) * (1 + Number.EPSILON)) / 100
            this.stats.boys.median = stats_disease[0].age_at_first_symp.boys.median
            this.stats.boys.median = Math.round((this.stats.boys.median * 100) * (1 + Number.EPSILON)) / 100
            this.stats.boys.iqr = stats_disease[0].age_at_first_symp.boys.iqr
            this.stats.boys.iqr = Math.round((this.stats.boys.iqr * 100) * (1 + Number.EPSILON)) / 100
            this.stats.girls.median = stats_disease[0].age_at_first_symp.girls.median
            this.stats.girls.median = Math.round((this.stats.girls.median * 100) * (1 + Number.EPSILON)) / 100
            this.stats.girls.iqr = stats_disease[0].age_at_first_symp.girls.iqr
            this.stats.girls.iqr = Math.round((this.stats.girls.iqr * 100) * (1 + Number.EPSILON)) / 100
            this.boxData_alt[0].y = stats_disease[0].age_at_first_symp.girls.array;
            this.boxData_alt[1].y = stats_disease[0].age_at_first_symp.boys.array;
            //console.log(this.boxLayout_alt.yaxis.range)
            this.boxLayout_alt.yaxis.range[1] = Math.max(stats_disease[0].age_at_first_symp.girls.median, stats_disease[0].age_at_first_symp.boys.median) + 
                                                2.6 * Math.max(stats_disease[0].age_at_first_symp.girls.iqr, stats_disease[0].age_at_first_symp.boys.iqr)
